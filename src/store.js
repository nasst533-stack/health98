...
function authErrorMessage(err) {
  ...
  if (code === "auth/email-already-in-use") return "이미 사용 중인 이름이에요. 다른 이름으로 가입해주세요.";
  if (code === "auth/weak-password") return "비밀번호가 너무 짧아요 (6자 이상 입력해주세요).";
  if (code === "auth/user-not-found" || code === "auth/invalid-credential") return "이름 또는 비밀번호가 올바르지 않아요.";
  if (code === "auth/wrong-password") return "비밀번호가 올바르지 않아요.";
  if (code === "auth/too-many-requests") return "시도가 너무 많았어요. 잠시 후 다시 시도해주세요.";
...
export function subscribePendingProfiles(callback) {
  return onSnapshot(
    query(collection(db, "profiles"), where("approved", "==", false)),
...
export function subscribeFavorites(profileId, callback) {
  return onSnapshot(
    query(collection(db, "favorites"), where("profileId", "==", profileId)),
    (snap) => callback(new Set(snap.docs.map((d) => d.data().exerciseId)))
  );
}
...
export function subscribeCustomExercises(callback) {
  return onSnapshot(
    query(collection(db, "customExercises"), orderBy("createdAt", "asc")),
    (snap) => callback(snap.docs.map((d) => ({ id: d.id, ...d.data(), builtin: false })))
  );
}
...
export function subscribeCustomFoods(callback) {
  return onSnapshot(
    query(collection(db, "customFoods"), orderBy("createdAt", "asc")),
    (snap) => callback(snap.docs.map((d) => ({ id: d.id, ...d.data(), builtin: false })))
  );
}
