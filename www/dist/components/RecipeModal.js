...
export function RecipeModal({ allFoods, initialRecipe, onClose, onSave }) {
    ...
    const [rows, setRows] = useState(() => initialRecipe && initialRecipe.ingredients && initialRecipe.ingredients.length > 0
    ...
    async function handleSubmit(e) {
        e.preventDefault();
        if (!dishName.trim() || totals.totalGrams <= 0)
            return;
        setSaving(true);
        setSaveError(null);
        try {
    ...
    return React.createElement("div", { className: "modal-backdrop", onClick: onClose }, React.createElement("form", { className: "modal", onClick: (e) => e.stopPropagation(), onSubmit: handleSubmit }, React.createElement("h2", null, initialRecipe ? "레시피 수정" : "만들어 먹음"), React.createElement("label", null, "요리 이름"), React.createElement("input", {
        ...
        placeholder: "예: 간장계란밥",
        autoFocus: true,
    }), React.createElement("label", null, "재료"), React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10 } }, rows.map((r) => React.createElement("div", { key: r.key, className: "note-box", style: { borderStyle: "solid" } }, !r.food &&
        ...
        }, f.name)), r.query.trim() && r.searchResults.length === 0 &&
            React.createElement("span", { className: "muted", style: { fontSize: 12.5 } }, "검색 결과가 없어요."))), r.food &&
        React.createElement(React.Fragment, null, React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 } }, React.createElement("strong", { style: { fontSize: 13.5 } }, r.food.name), React.createElement("button", { type: "button", className: "history-delete", onClick: () => updateRow(r.key, { food: null }) }, "재료 변경")), React.createElement("div", { style: { display: "flex", gap: 8 } }, React.createElement("input", {
            type: "number",
            step: "0.1",
            min: 0,
            value: r.qty,
            ...
            React.createElement("option", { value: "count" }, `${r.food.unitLabel || "개"} (1${r.food.unitLabel || "개"}≈${r.food.unitGrams}g)`), React.createElement("option", { value: "g" }, "g"), React.createElement("option", { value: "tbsp" }, "큰술 (≈15g)"), React.createElement("option", { value: "tsp" }, "작은술 (≈5g)"), React.createElement("option", { value: "cup" }, "컵 (≈200g)")))))), React.createElement("button", { type: "button", className: "add-set-btn", onClick: addRow, style: { margin: 0 } }, "+ 재료 추가")), React.createElement("div", { className: "kcal-summary", style: { marginTop: 14 } }, React.createElement("div", null, React.createElement("div", { className: "muted", style: { fontSize: 12 } }, `총 ${totals.totalGrams}g`), React.createElement("strong", null, `${totals.kcal} kcal`)), React.createElement("div", { className: "muted", style: { fontSize: 12.5, textAlign: "right" } }, `탄 ${totals.carb != null ? totals.carb : "-"}g`, React.createElement("br"), `단 ${totals.protein != null ? totals.protein : "-"}g · 지 ${totals.fat != null ? totals.fat : "-"}g`)), React.createElement("div", { className: "modal-actions" }, React.createElement("button", { type: "button", className: "btn-secondary", onClick: onClose }, "취소"), React.createElement("button", { type: "submit", className: "btn-primary", disabled: saving || !dishName.trim() || totals.totalGrams <= 0 }, saving ? "저장 중..." : "오늘 기록에 추가")), saveError && React.createElement("p", { style: { color: "var(--danger)", fontSize: "12px", marginTop: 8 } }, saveError)));
}
