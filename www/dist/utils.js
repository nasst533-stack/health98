...
export function weekDates(offset) {
    const arr = [];
    for (let i = 6; i >= 0; i--)
        arr.push(daysAgoStr(offset * 7 + i));
    return arr;
}
...
// ---------------------------------------------------------------------
export function buildRecommendation(goal, weightDiff, avgKcal, recentVolume, recentCardioMin, muscleDiff, fatPctDiff, macroGap) {
    let base;
    if (!goal) {
        return {
            title: "목표를 먼저 설정해주세요",
